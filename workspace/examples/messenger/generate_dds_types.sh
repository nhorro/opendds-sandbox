...
OPENDDS_IDL=$(command -v opendds_idl)

if [[ -z "$TAO_IDL" || -z "$OPENDDS_IDL" ]]; then
...
for idl_path in "$SRC_DIR"/*.idl; do
  ...
  $TAO_IDL -I"$DDS_ROOT" -I"$TAO_ROOT" "$idl_path" -o "$OUT_DIR"

  # Paso 2: generar TypeSupport IDL y código DDS
  $OPENDDS_IDL "$idl_path" -o "$OUT_DIR"

  # Paso 3: procesar el IDL generado por OpenDDS
  ts_idl="$OUT_DIR/${base_name}TypeSupport.idl"
  if [[ -f "$ts_idl" ]]; then
    $TAO_IDL -I"$DDS_ROOT" -I"$TAO_ROOT" -I"$SRC_DIR" "$ts_idl" -o "$OUT_DIR"
  else
    echo "WARNING: No se generó ${base_name}TypeSupport.idl"
  fi
done
